...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="PACS")#TerraIncognita
    parser.add_argument("--algorithm", type=str, default="AOCI")
    parser.add_argument("--KD_type", type=str, default="CIRL")#{FACT,CIRL,ConY}
    parser.add_argument("--lam_const", type=float, default=2.0)
    parser.add_argument("--Fourier", action="store_true", default=False, help="Fourier data enhancement")
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--warmup_epoch", type=int, default=100)
    parser.add_argument("--weight_decay", type=float, default=1e-6)
    parser.add_argument("--lam_dis", type=str, default="uniform")#{beta,uniform,gaussian}
    parser.add_argument("--times", type=int, default=1,help="how many times enhancement data")#[1,2,3]
    parser.add_argument("--seed_type", type=str, default="randn")
    ...
    if args.dataset=="DomainNet":
        env_nums = 6
    hparams["KD_type"] = args.KD_type
    hparams["lam_const"] = args.lam_const
    hparams["Fourier"] = args.Fourier
    hparams["batch_size"] = args.batch_size
    hparams["warmup_epoch"] = args.warmup_epoch
    hparams["weight_decay"] = args.weight_decay
    hparams["times"] = args.times
    
    sel_seeds = np.random.choice(1000, 3) if args.seed_type=="randn" else [2,1,0]
    for dist in ["gauss","beta","uniform"]:
        args.lam_dis = dist
        SWADs = [[] for i in range(env_nums)]
        for trial_seed in sel_seeds:
            args.test_envs = None
            args.trial_seed = trial_seed # seed
            ...
            for test_env in args.test_envs:
                ...
                for k, v in res.items():
                    results[k].append(v)
                    if k=="SWAD":
                        SWADs[test_env[0]].append(v)  # 每个testenv下记录准确率
            # log summary table
            logger.info("=== Summary ===")
        ...
        env_mean = np.mean(SWADs,axis = 1)
        env_std = np.std(SWADs,axis=1)
        total = np.mean(SWADs,axis=0)
